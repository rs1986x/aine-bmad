...
- [x] [Review][Decision] (resolved → committed as `c3d69ca`) Story is `done` and claims its changes landed in `ec90cf5`/`009bde0`, but the entire review-fix changeset is still uncommitted — `git status` shows `.github/workflows/ci.yml`, `backend/{eslint.config.mjs,package.json,tsconfig.json}`, `frontend/{eslint.config.js,package.json}`, `docker-compose.test.yml`, `e2e/package.json` modified, and `e2e/tsconfig.json` + `e2e/eslint.config.js` UNTRACKED. On a clean checkout of `HEAD`, the e2e `lint`/`typecheck` CI steps break (no config files) and the port-5432, vitest-globals, concurrency, and engines patches are absent. The applied prior-review patches exist only in the working tree. [_bmad-output/implementation-artifacts/1-1-…md, e2e/, working tree]
- [x] [Review][Patch] (applied) Backend `typecheck` skips test files — `backend/tsconfig.json` `exclude` lists `**/*.test.ts` + `src/__tests__` (added to keep tests out of the `build` emit), but the same config drives `tsc --noEmit`, so backend test type errors are never caught in CI (asymmetric with frontend, whose `tsconfig.app.json` includes `src`). **Fix: removed the test excludes from `tsconfig.json` (so `typecheck` covers tests) and added `tsconfig.build.json` (extends + re-adds the excludes) used by the `build` script. Verified: `tsc --noEmit` clean incl. tests; `tsc -p tsconfig.build.json` emits only `dist/index.js`.** [backend/tsconfig.json, backend/tsconfig.build.json, backend/package.json]
- [x] [Review][Patch] (applied) CI concurrency group does not dedupe push vs open-PR — `group: ci-…-${{ github.ref }}` gives a branch push (`refs/heads/<b>`) and its PR (`refs/pull/N/merge`) different keys, so both still run; the comment claims it prevents exactly that. **Fix: group key now `github.head_ref || github.ref` so a PR and its source-branch push share one group; comment corrected.** [.github/workflows/ci.yml:9-11]
